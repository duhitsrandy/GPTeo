...
### Development
- `npm run dev` - Start development server with Turbopack (automatically kills port 3000 first)
- `npm run dev:https` - Start development server with HTTPS (useful for testing webhooks/OAuth)
- `npm run build` - Build for production
- `npm run start` - Start production server

**Note:** Both dev scripts automatically kill any existing process on port 3000 before starting. This prevents "port already in use" errors from zombie processes.
